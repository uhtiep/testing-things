...
class Player {
  ...
  shoot() {
    if (this.powerups.includes("Rapid Fire")) {
      this.shootBullet();
      setTimeout(() => this.shootBullet(), 100); // Faster firing
    } else {
      this.shootBullet();
    }
  }
...
function startGame() {
  ...
  powerupInterval = setInterval(createPowerup, 5000); // Create powerups every 5 seconds
}
...
function handlePowerup(powerup) {
  if (powerup.type === "Health") {
    ...
    player.powerups.push("Rapid Fire");
  }
}
